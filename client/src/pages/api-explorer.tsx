...
export default function ApiExplorer() {
  ...
  React.useEffect(() => {
    if (selectedEndpoint) {
      // Use the request example from the database if available
      const parameters = selectedEndpoint.parameters as any[] || [];
      const sampleRequest = selectedEndpoint.requestExample || 
        (parameters.length > 0 ? 
          JSON.stringify(
            parameters.reduce((acc: any, param: any) => {
              acc[param.name] = param.example || (param.type === 'number' ? 0 : 
                param.type === 'boolean' ? false : 'string');
              return acc;
            }, {}), null, 2
          ) : 
          '{}'
...
{`{
  ...
  "status": "approved"
                        ...
                        {selectedEndpoint.parameters && (selectedEndpoint.parameters as any[]).length > 0 && (
                          <div>
                            ...
                            <div className="overflow-x-auto">
                              <table className="w-full border border-gray-200 text-xs">
                                ...
                                <tbody>
                                  {(selectedEndpoint.parameters as any[]).map((param: any, index: number) => (
                                    <tr key={index}>
                                      <td className="border border-gray-200 p-2 font-mono">{param.name}</td>
                                      <td className="border border-gray-200 p-2">{param.type}</td>
                                      <td className="border border-gray-200 p-2">
                                        <span className={param.required ? "text-red-600" : "text-gray-600"}>
                                          {param.required ? "Required" : "Optional"}
                                        </span>
                                      </td>
                                      <td className="border border-gray-200 p-2">{param.description}</td>
                                    </tr>
                                  ))}
                                </tbody>
                              </table>
                        ...
                        {selectedEndpoint.responses && (selectedEndpoint.responses as any[]).length > 0 && (
                          <div>
                            ...
                            <div className="overflow-x-auto">
                              <table className="w-full border border-gray-200 text-xs">
                                <thead>
                                  <tr className="bg-gray-50">
                                    <th className="border border-gray-200 p-2 text-left">Status Code</th>
                                    <th className="border border-gray-200 p-2 text-left">Description</th>
                                    <th className="border border-gray-200 p-2 text-left">Example</th>
                                  </tr>
                                </thead>
                                <tbody>
                                  {(selectedEndpoint.responses as any[]).map((response: any, index: number) => (
                                    <tr key={index}>
                                      <td className="border border-gray-200 p-2 font-mono">{response.statusCode}</td>
                                      <td className="border border-gray-200 p-2">{response.description}</td>
                                      <td className="border border-gray-200 p-2">
                                        {response.example && (
                                          <pre className="text-xs">{response.example}</pre>
                                        )}
                                      </td>
                                    </tr>
                                  ))}
                        ...
                        {(selectedEndpoint.requestExample || selectedEndpoint.responseExample) && (
                          <div>
                            <h4>Examples</h4>
                        ...
                        {selectedEndpoint.documentation && (
                          <div>
                            ...
                            <div className="prose prose-sm max-w-none">
                              <pre className="whitespace-pre-wrap text-sm">{selectedEndpoint.documentation}</pre>
                            </div>
                          </div>
                        )}
